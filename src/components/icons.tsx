/** Octicon-style SVG icons, 14px, single-color (fill: currentColor). */

export function IcoCopy() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 0 1 0 1.5h-1.5a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-1.5a.75.75 0 0 1 1.5 0v1.5A1.75 1.75 0 0 1 9.25 16h-7.5A1.75 1.75 0 0 1 0 14.25Z"/><path d="M5 1.75C5 .784 5.784 0 6.75 0h7.5C15.216 0 16 .784 16 1.75v7.5A1.75 1.75 0 0 1 14.25 11h-7.5A1.75 1.75 0 0 1 5 9.25Zm1.75-.25a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-7.5a.25.25 0 0 0-.25-.25Z"/></svg>;
}

export function IcoCheck() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z"/></svg>;
}

export function IcoTrash() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M11 1.75V3h2.25a.75.75 0 0 1 0 1.5H2.75a.75.75 0 0 1 0-1.5H5V1.75C5 .784 5.784 0 6.75 0h2.5C10.216 0 11 .784 11 1.75ZM4.496 6.675l.66 6.6a.25.25 0 0 0 .249.225h5.19a.25.25 0 0 0 .249-.225l.66-6.6a.75.75 0 0 1 1.492.149l-.66 6.6A1.748 1.748 0 0 1 10.595 15h-5.19a1.75 1.75 0 0 1-1.741-1.575l-.66-6.6a.75.75 0 1 1 1.492-.15ZM6.5 1.75V3h3V1.75a.25.25 0 0 0-.25-.25h-2.5a.25.25 0 0 0-.25.25Z"/></svg>;
}

export function IcoKeyboard() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M0 4.75C0 3.784.784 3 1.75 3h12.5c.966 0 1.75.784 1.75 1.75v6.5A1.75 1.75 0 0 1 14.25 13H1.75A1.75 1.75 0 0 1 0 11.25Zm1.75-.25a.25.25 0 0 0-.25.25v6.5c0 .138.112.25.25.25h12.5a.25.25 0 0 0 .25-.25v-6.5a.25.25 0 0 0-.25-.25ZM2 7h1v1H2Zm3 0h1v1H5Zm3 0h1v1H8Zm3 0h1v1h-1ZM5 5h1v1H5Zm3 0h1v1H8Zm3 0h1v1h-1ZM2 9h5v1H2Zm7 0h5v1H9Zm-2 0h1v1H7Z"/></svg>;
}

export function IcoBook() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M0 1.75A.75.75 0 0 1 .75 1h4.253c1.227 0 2.317.59 3 1.501A3.743 3.743 0 0 1 11.006 1h4.245a.75.75 0 0 1 .75.75v10.5a.75.75 0 0 1-.75.75h-4.507a2.25 2.25 0 0 0-1.591.659l-.622.621a.75.75 0 0 1-1.06 0l-.622-.621A2.25 2.25 0 0 0 5.258 13H.75a.75.75 0 0 1-.75-.75Zm7.251 10.324.004-5.073-.002-2.253A2.25 2.25 0 0 0 5.003 2.5H1.5v9h3.757a3.75 3.75 0 0 1 1.994.574ZM8.755 4.75l-.004 7.322a3.752 3.752 0 0 1 1.992-.572H14.5v-9h-3.495a2.25 2.25 0 0 0-2.25 2.25Z"/></svg>;
}

export function IcoLightning() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M9.504.43a1.516 1.516 0 0 1 .437 1.688L8.218 6h4.37a1.358 1.358 0 0 1 1.087 2.172l-.639.795-.031.027-.7 1.047a1.355 1.355 0 0 1-.95.572l-3.773.34L6.457 16.3a1.353 1.353 0 0 1-1.235.7 1.359 1.359 0 0 1-1.246-.845l-.37-.95a1.358 1.358 0 0 1 .065-1.046l1.92-3.616H2.016a1.358 1.358 0 0 1-1.013-2.27L9.504.43Z"/></svg>;
}

export function IcoSun() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M8 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm0-9.5a.75.75 0 0 1-.75-.75V.25a.75.75 0 0 1 1.5 0v1.5A.75.75 0 0 1 8 2.5Zm0 13a.75.75 0 0 1-.75-.75v-1.5a.75.75 0 0 1 1.5 0v1.5a.75.75 0 0 1-.75.75ZM2.5 8a.75.75 0 0 1-.75.75H.25a.75.75 0 0 1 0-1.5h1.5A.75.75 0 0 1 2.5 8Zm13 0a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1 0-1.5h1.5a.75.75 0 0 1 .75.75ZM3.94 3.94a.75.75 0 0 1 0 1.06L2.88 6.06a.75.75 0 1 1-1.06-1.06l1.06-1.06a.75.75 0 0 1 1.06 0Zm9.19 9.19a.75.75 0 0 1 0 1.06l-1.06 1.06a.75.75 0 1 1-1.06-1.06l1.06-1.06a.75.75 0 0 1 1.06 0ZM3.94 12.06a.75.75 0 0 1-1.06 0l-1.06-1.06a.75.75 0 1 1 1.06-1.06l1.06 1.06a.75.75 0 0 1 0 1.06Zm9.19-9.19a.75.75 0 0 1-1.06 0l-1.06-1.06a.75.75 0 1 1 1.06-1.06l1.06 1.06a.75.75 0 0 1 0 1.06Z"/></svg>;
}

export function IcoMoon() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M9.598 1.591a.75.75 0 0 1 .785-.175 7 7 0 1 1-8.967 8.967.75.75 0 0 1 .961-.96 5.5 5.5 0 0 0 7.046-7.046.75.75 0 0 1 .175-.786Zm1.616 1.945a7 7 0 0 1-7.678 7.678 5.5 5.5 0 1 0 7.678-7.678Z"/></svg>;
}

export function IcoPlus() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M7.75 2a.75.75 0 0 1 .75.75V7h4.25a.75.75 0 0 1 0 1.5H8.5v4.25a.75.75 0 0 1-1.5 0V8.5H2.75a.75.75 0 0 1 0-1.5H7V2.75A.75.75 0 0 1 7.75 2Z"/></svg>;
}

export function IcoSidebar() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M1.75 2A1.75 1.75 0 0 0 0 3.75v8.5C0 13.216.784 14 1.75 14h12.5A1.75 1.75 0 0 0 16 12.25v-8.5A1.75 1.75 0 0 0 14.25 2Zm-.25 1.75a.25.25 0 0 1 .25-.25H5v9H1.75a.25.25 0 0 1-.25-.25Zm5 8.75v-9h7.75a.25.25 0 0 1 .25.25v8.5a.25.25 0 0 1-.25.25Z"/></svg>;
}

export function IcoDownload() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M2.75 14A1.75 1.75 0 0 1 1 12.25v-2.5a.75.75 0 0 1 1.5 0v2.5c0 .138.112.25.25.25h10.5a.25.25 0 0 0 .25-.25v-2.5a.75.75 0 0 1 1.5 0v2.5A1.75 1.75 0 0 1 13.25 14Z"/><path d="M7.25 7.689V2a.75.75 0 0 1 1.5 0v5.689l1.97-1.97a.749.749 0 1 1 1.06 1.06l-3.25 3.25a.749.749 0 0 1-1.06 0L4.22 6.779a.749.749 0 1 1 1.06-1.06l1.97 1.97Z"/></svg>;
}

export function IcoPencil() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M11.013 1.427a1.75 1.75 0 0 1 2.474 0l1.086 1.086a1.75 1.75 0 0 1 0 2.474l-8.61 8.61c-.21.21-.47.364-.756.445l-3.251.93a.75.75 0 0 1-.927-.928l.929-3.25c.081-.286.235-.547.445-.758ZM12.09 2.487a.25.25 0 0 0-.354 0L9.72 4.502l1.779 1.779 2.016-2.016a.25.25 0 0 0 0-.354Zm-1.474 4.253L8.838 4.96 3.545 10.253a.243.243 0 0 0-.064.108l-.558 1.953 1.953-.558a.243.243 0 0 0 .108-.064Z"/></svg>;
}

export function IcoSave() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M17 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"/></svg>;
}

export function IcoCode() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M4.72 3.22a.75.75 0 0 1 1.06 1.06L2.06 8l3.72 3.72a.75.75 0 1 1-1.06 1.06L.47 8.53a.75.75 0 0 1 0-1.06Zm6.56 0a.75.75 0 1 0-1.06 1.06L13.94 8l-3.72 3.72a.75.75 0 1 0 1.06 1.06l4.25-4.25a.75.75 0 0 0 0-1.06Z"/></svg>;
}

export function IcoGear() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a.75.75 0 0 1 .75.75v.716a5.98 5.98 0 0 1 1.598.663l.507-.507a.75.75 0 1 1 1.06 1.06l-.507.507c.29.47.514 1.024.646 1.598h.716a.75.75 0 0 1 0 1.5h-.716a5.98 5.98 0 0 1-.646 1.598l.507.507a.75.75 0 1 1-1.06 1.06l-.507-.507a5.98 5.98 0 0 1-1.598.646v.716a.75.75 0 0 1-1.5 0v-.716a5.98 5.98 0 0 1-1.598-.646l-.507.507a.75.75 0 1 1-1.06-1.06l.507-.507a5.98 5.98 0 0 1-.646-1.598H2.25a.75.75 0 0 1 0-1.5h.716c.132-.574.356-1.128.646-1.598l-.507-.507a.75.75 0 0 1 1.06-1.06l.507.507A5.98 5.98 0 0 1 6.25 2.466V1.75A.75.75 0 0 1 7 1Zm.75 5a2.25 2.25 0 1 0 0 4.5 2.25 2.25 0 0 0 0-4.5Z"/></svg>;
}

export function IcoChat() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor"><path d="M1.5 2.75C1.5 1.784 2.284 1 3.25 1h9.5c.966 0 1.75.784 1.75 1.75v7.5A1.75 1.75 0 0 1 12.75 12H7.06l-2.573 2.573A.25.25 0 0 1 4.06 14.4V12h-.81A1.75 1.75 0 0 1 1.5 10.25Z"/></svg>;
}

export function IcoFlask() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M6.25 1A.75.75 0 0 0 6.25 2.5v3.06l-3.7 6.42A1.5 1.5 0 0 0 3.85 14h8.3a1.5 1.5 0 0 0 1.3-2.02l-3.7-6.42V2.5a.75.75 0 0 0 0-1.5h-3.5Zm.5 5.5V2.5h2.5v4a.75.75 0 0 0 .1.375L11.5 10h-7l2.15-3.725A.75.75 0 0 0 6.75 6.5Z"/></svg>;
}

export function IcoTray() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M2.5 1.75A.75.75 0 0 1 3.25 1h9.5a.75.75 0 0 1 .75.75v3.5a.75.75 0 0 1-.75.75h-.5v6.5A1.75 1.75 0 0 1 10.5 14h-5a1.75 1.75 0 0 1-1.75-1.75V6h-.5a.75.75 0 0 1-.75-.75Zm2.75 4.5v6.5c0 .138.112.25.25.25h5a.25.25 0 0 0 .25-.25V6.25ZM4 4.75h8V2.5H4Z"/></svg>;
}

export function IcoX() {
  return <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor"><path d="M3.72 3.72a.75.75 0 0 1 1.06 0L8 6.94l3.22-3.22a.749.749 0 0 1 1.275.326.749.749 0 0 1-.215.734L9.06 8l3.22 3.22a.749.749 0 0 1-.326 1.275.749.749 0 0 1-.734-.215L8 9.06l-3.22 3.22a.751.751 0 0 1-1.042-.018.751.751 0 0 1-.018-1.042L6.94 8 3.72 4.78a.75.75 0 0 1 0-1.06Z"/></svg>;
}
